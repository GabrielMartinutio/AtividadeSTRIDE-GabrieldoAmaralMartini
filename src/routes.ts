...
router.post("/transactions/transfer", (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "falha ao realizar transferência" });
  }
});
...
router.post("/reports/generate", (req: Request, res: Response) => {
  ...
  const allowedReports: Record<string, string> = {
    summary: "relatório resumido gerado com sucesso",
    balance: "relatório de saldo gerado com sucesso",
  };

  if (!(type in allowedReports)) {
    return res.status(400).json({ error: "tipo de relatório inválido" });
  }

  return res.status(200).json({
    message: "relatório gerado",
    output: allowedReports[type],
  });
});
